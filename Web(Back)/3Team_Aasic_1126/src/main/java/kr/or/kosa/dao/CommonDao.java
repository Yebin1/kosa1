...
//	}
	...
	public int deleteOk(String idx, String userid) {
		try {
			...
			// String sql_reply = "update board set replystatus=0 where idx_fk=?"; // 테이블에
			// cascade 처리하면 알아서 자식도 지워짐
...
//				}
	...
	// 핫게시판~~~
	public List<Board> hotBoardList(String boardname, String title) {

		System.out.println("핫게쿼리문DAO시작");

		try {
			conn = ds.getConnection();
			sql1 = "select boardname, title, idx from "
					+ "(select boardname, title from board order by lovenum desc) where rownum <= 3";
			pstmt1 = conn.prepareStatement(sql1);
