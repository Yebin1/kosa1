...
public class MemberDao {
	...
	//private Map<Reply, Board> myReplyList;
...
//						int lovenum = rs1.getInt("lovenum");
	...
	/*
	public List<Board, Reply> myReplyList(int cpage, int pagesize, String userid) {
		try {
			conn = ds.getConnection();
			sql1 ="select * from "
					+ "(select rownum, r.*, b.boardname, b.title from reply r, board b "
					+ "where r.idx = b.idx "
					+ "order by replynum desc) "
					+ "where rownum <= ? and rownum >= ? and replystatus =1 and userid=?";
			...
			while (rs1.next()) {
				reply = new Reply();
				board = new Board();
				reply.setReplynum(rs1.getInt("replynum")); // 댓글 인덱스
				board.setIdx(rs1.getInt("idx")); // 댓글 달린 글 인덱스
				board.setBoardname(rs1.getString("boardname"));
				board.setTitle(rs1.getString("title"));
				reply.setContent(rs1.getString("content"));
				reply.setRefernum(rs1.getInt("refernum"));
				reply.setReplydate(rs1.getDate("replydate"));
											
				myReplyList.add(reply);
				myReplyList.add(board);
				
				System.out.println("DAO HashMap reply: " + reply);
				System.out.println("DAO HashMap board: " + board);
				System.out.println();
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	*/
	...
	public List<Object> myReplyList(int cpage, int pagesize, String userid) {
		try {
			conn = ds.getConnection();
			sql1 ="select rb.replynum as replynum, rb.idx as idx, rb.boardname as boardname, rb.title as title, rb.content as content, rb.refernum as refernum, to_char(rb.replydate) as replydate from "
					+ "(select rownum as num, r.*, b.boardname, b.title from reply r, board b "
					+ "where r.idx = b.idx "
					+ "order by replynum desc)rb "
					+ "where num <= ? and num >= ? and replystatus =1 and userid=?";
	...
	// 특정 회원 댓글 총 건수 구하기
		public int myReplyCount(String userid) {
			try {
				conn = ds.getConnection();
				sql1 = "select count(*) cnt from reply where userid = ?";
				pstmt1 = conn.prepareStatement(sql1);
				pstmt1.setString(1, userid);
