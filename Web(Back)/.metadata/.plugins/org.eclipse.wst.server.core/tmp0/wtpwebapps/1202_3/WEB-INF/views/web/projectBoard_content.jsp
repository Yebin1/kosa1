...
$(function(){
 ...
 //북마크 노다영 바보.
    ...
    mapOption = {  center: new kakao.maps.LatLng($('#latitudemap').val(), $('#logitudemap').val()), // 지도의 중심좌표
        level: 3 // 지도의 확대 레벨
    };
				...
				<table class="table" width="95%" align="center">
					...
					<tr>
						...
						<td><c:choose>
							...
							<td>
                        		...
                        		<c:forEach var="recruitboard" items="${recruitboardList}">
                        			...
                        			<input type="hidden" id="latitudemap" name="latitudemap" value="${recruitboard.latitudemap}"/>
                        			<input type="hidden" id="logitudemap" name="logitudemap" value="${recruitboard.logitudemap}"/>
								</c:forEach>
						...
						<tr>
							...
							<c:forEach var="recruitboard" items="${recruitboardList}">
							<td colspan="2"><div><b>장소</b></div> <div>${recruitboard.placename}</div> </td>
							</c:forEach>
							</tr>
							<tr  height="230">
							<td colspan="2" align="left">${fn:replace(board.content, newLineChar,"<br>")}
							<td colspan="2" align="left" 
							style="border-left: solid; border-color:#dcdcdc; "><!-- 베이지색 : #fff5ee -->
					...
					<tr>
						...
						<td colspan="3" align="left">
							<c:choose>
								<c:when test="${recruitboard.apply != null && fn:length(apply) > 10}">
									<a href="${recruitboard.apply}">${fn:substring(apply,0,15)}...</a>
								</c:when>
								<c:otherwise>
									<a href="${recruitboard.apply}">${recruitboard.apply}</a>
								</c:otherwise>
							</c:choose></td>
