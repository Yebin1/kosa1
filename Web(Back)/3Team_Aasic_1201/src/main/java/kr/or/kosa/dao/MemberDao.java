...
public class MemberDao {
	...
	public int projectWrite(Recruitboard recruitboard, Board board) {
		...
		try {
			...
			sql1 = "insert all "
					...
					+ "values(board_idx.nextval,?,?,?,?,sysdate,0,0,1,?) "
					+ "into recruitboard(idx,startdate,enddate,latitudemap,logitudemap,apply) "
	...
	public int projectWrite(Recruitboard recruitboard, Board board, Files file) {
		try {
			...
			pstmt1.setDouble(9, recruitboard.getLogitudemap());
			pstmt1.setString(10, recruitboard.getApply());
			pstmt1.setString(11, file.getFilename());
			pstmt1.setInt(12, file.getFilesize());
	...
	public List<Bookmarks> BookmarksList(int cpage, int pagesize, String userid) {
		sql1 = "SELECT *  FROM "
				...
				+ "(SELECT bookmarks.userid, bookmarks.idx, bookmarks.writerid, bookmarks.title, bookmarks.lovenum, bookmarks.writedate, bookmarks.boardname, board.boardstatus FROM bookmarks, board "
				+ "where bookmarks.idx=board.idx and boardstatus=1 and bookmarks.userid=? "
				+ "ORDER BY idx DESC) bb) "
				+ "WHERE RNUM <=?  AND RNUM >=?";
	...
	public List<Recruitboard> recruitboardList(int idx) {
		...
		try {
			...
			conn = ds.getConnection();
			sql1 = "select idx,startdate,enddate,latitudemap,logitudemap,apply from recruitboard where idx = ?";
			pstmt1 = conn.prepareStatement(sql1);
			pstmt1.setInt(1, idx);
			rs1 = pstmt1.executeQuery();
			...
			while (rs1.next()) {
				...
				recruitboard.setLatitudemap(rs1.getInt("latitudemap"));
				recruitboard.setLogitudemap(rs1.getInt("logitudemap"));
