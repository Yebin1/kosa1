...
public class MemberDao {
	...
	public int totalBoardCount() {
		try {
			conn = ds.getConnection();
			sql1 = "select count(*) cnt from board and boardstatus=1";
			pstmt1 = conn.prepareStatement(sql1);
			rs1 = pstmt1.executeQuery();
			if (rs1.next()) {
				totalcount = rs1.getInt("cnt");
	...
	public int totalLoveCount(String userid) {
		...
		try {
			...
			sql1 = "select sum(lovenum) from board where userid=?  and boardstatus=1";
			pstmt1 = conn.prepareStatement(sql1);
...
//						int lovenum = rs1.getInt("lovenum");
	...
	public List<Board> headerSearch(int cpage, int pagesize, String boardheader) {
		...
		try {
			...
			pstmt1 = conn.prepareStatement(sql1);
			pstmt1.setInt(1, board.getIdx());
			pstmt1.setString(2, board.getUserid());
			rs1 = pstmt1.executeQuery();
	...
	// 말머리 검색 건수
		public int headerCount(String boardheader) {
			try {
				conn = ds.getConnection();
				sql1 = "select count(*) cnt from board where boardheader LIKE '%?%' and boardstatus=1";
				pstmt1 = conn.prepareStatement(sql1);
