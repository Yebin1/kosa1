...
<body>
			...
			<table class="table"><!-- width="80%" border="1" cellspacing="0" align="center" -->
				...
				<tr>
					<td colspan="3" align="center" >
						<!--이전 링크 --> 
						<c:if test="${cpage > 1}">
							<a href="boardList.user?boardname=${board.boardname}&cp=${cpage-1}&ps=${pagesize}&boardname=${board.boardname}">이전</a>
						</c:if>
						<!-- page 목록 나열하기 -->
						<c:forEach var="i" begin="1" end="${pagecount}" step="1">
							<c:choose>
								...
								<c:otherwise>
									<a href="boardList.user?boardname=${board.boardname}&cp=${i}&ps=${pagesize}&boardname=${board.boardname}">[${i}]</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<!--다음 링크 --> 
						<c:if test="${cpage < pagecount}">
							<a href="boardList.user?boardname=${board.boardname}&cp=${cpage+1}&ps=${pagesize}&boardname=${board.boardname}">다음</a>
						</c:if>
					</td>
	...
	<div>
		<%-- <li><a href="boardWrite.user?boardname=${boardname}">글쓰기</a></li> --%>
		 <button type="button" class="btn btn-primary rounded-pill m-2" onclick="location.href='boardWrite.user?boardname=${board.boardname}';">글쓰기</button>
	</div>
