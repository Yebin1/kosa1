...
public class UserController extends HttpServlet {
	...
	private void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		} else if (urlcommand.equals("/userEditInfo.user")) { // 회원 정보 수정 (view만 보여주기)
			forward = new ActionForward();
			forward.setRedirect(false);
			forward.setPath("/WEB-INF/views/web/userInfoEdit.jsp");

		} else if (urlcommand.equals("/update.user")) { // 회원 정보 업데이트
			action = new UserInfoEditService();
			forward = action.execute(request, response);

		} else if (urlcommand.equals("/updateView.user")) { // 게시글 수정
			forward = new ActionForward();
			forward.setRedirect(false);
			forward.setPath("/WEB-INF/views/web/edit.jsp");

		} else if (urlcommand.equals("/updateOK.user")) { // 게시글 수정 업데이트
			action = new UpdateService();
			forward = action.execute(request, response);

		} else if (urlcommand.equals("/boardList.user")) { // 게시글 리스트 출력
		...
		} else if (urlcommand.equals("/boardWriteOK.user")) { // 게시글 작성 업데이트
			action = new BoardAddService();
			forward = action.execute(request, response);

		} else if (urlcommand.equals("/boardContent.user")) { // 글 내용 보기
