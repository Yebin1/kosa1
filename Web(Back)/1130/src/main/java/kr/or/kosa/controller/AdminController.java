...
import kr.or.kosa.service.admin.StatusZeroList;
...
//		}
		...
		} else if (urlcommand.equals("/statusZero.admin")) { // 삭제 대기글 목록
			action = new StatusZeroList();
			forward = action.execute(request, response);

		} else if (urlcommand.equals("/userList.admin")) { // 회원 목록
			action = new Search();
			forward = action.execute(request, response);
