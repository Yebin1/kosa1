...
import kr.or.kosa.service.common.HotBoardList;
...
import kr.or.kosa.service.common.NoticeBoardList;
import kr.or.kosa.service.common.SearchWordService;
    ...
    private void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {    	
    	...
    	if(urlcommand.equals("/index.do")) {   //메인페이지(홈버튼)
		   ...
		   action = new UserDeleteOk();
 		   forward = action.execute(request, response);     
	    }else if(urlcommand.equals("/hotList.do")) { // 핫게시판
			   action = new HotBoardList ();
	 		   forward = action.execute(request, response);     
		}else if(urlcommand.equals("/noticeList.do")) { // 공지게시판
			   action = new NoticeBoardList ();
	 		   forward = action.execute(request, response);     
		}else if(urlcommand.equals("/searchWord.do")) { // 검색어 > 전체조회(제목, 내용)
		       action = new SearchWordService();
		 	   forward = action.execute(request, response);
	     }
