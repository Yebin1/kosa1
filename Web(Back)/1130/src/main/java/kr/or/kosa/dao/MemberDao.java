...
public class MemberDao {
	...
	public List<Board> headerAll(int cpage, int pagesize, String boardname) {
		...
		try {
			...
			sql1 = "select * from (select rownum rnum, b.* from "
					+ "(select idx, userid, boardname, title, writedate, boardstatus, boardheader, lovenum, readnum from board "
					+ "where boardname=? and boardheader like '%%' and boardstatus =1 order by idx desc) b) "
					+ "where rnum >=? and rnum <=?";
	...
	/*
	 * 말머리 건수 카운트 public int headerTotalCount(String boardname, String boardheader)
	 * { try { conn = ds.getConnection(); sql1 =
	 * "select count(*) cnt from board where boardname=? and boardstatus=1 and boardheader like ?"
	 * ;
	 * 
	 * pstmt1 = conn.prepareStatement(sql1); pstmt1.setString(1, boardname);
	 * pstmt1.setString(2, "%" + boardheader + "%"); rs1 = pstmt1.executeQuery();
	 * 
	 * if (rs1.next()) { totalcount = rs1.getInt("cnt"); } } catch (Exception e) {
	 * e.printStackTrace(); } finally { try { pstmt1.close(); rs1.close();
