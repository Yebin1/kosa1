			...
			<table class="table"><!-- width="80%" border="1" cellspacing="0" align="center" -->
				...
				<tr>
					<td colspan="3" align="center" >
						<!--이전 링크 --> 
						<c:if test="${cpage > 1}">
							<a href="searchWord.do?boardname=${boardname}&cp=${cpage-1}&ps=${pagesize}&searchWord=${searchWord}">이전</a>
						</c:if>
						<!-- page 목록 나열하기 -->
						<c:forEach var="i" begin="1" end="${pagecount}" step="1">
							<c:choose>
								...
								<c:otherwise>
								<a href="searchWord.do?boardname=${boardname}&cp=${i}&ps=${pagesize}&searchWord=${searchWord}">[${i}]</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<!--다음 링크 --> 
						<c:if test="${cpage < pagecount}">
							<a href="searchWord.do?boardname=${boardname}&cp=${cpage+1}&ps=${pagesize}&searchWord=${searchWord}">다음</a>
						</c:if>
					</td>
