...
/*    $(document).on("click", "#clickDiv", function(){
 ...
 //북마크 노다영 바보.
  ...
  //댓글 끌어올리기 비동기 
    $(document).on("click","#replycompare",function() {
        $.ajax({
           url : "replyCompare.user",
           type : "POST",
           dataType : "html",
           data : {
         	 replyContent : $(this).attr("seq"),
              idx : $(this).attr("idx")  //원글의 인덱스
           },
           context: this,
           success : function(data){
         	  console.log(data);
                  $('#replyup').empty(); 
                  $('#replyup').append(data); //비어있는 <div>인 replyup에다 선택 댓글 붙여줌(append)
           },
           error : function(request,status,error){
              alert("code:" +request.status+"\n" + "message:"+request.responseText+"\n"+"error:"+error);
           }});
		...
		<div style="padding-top: 30px; text-align: center">
			<center>
				...
				<table class="table" width="95%" align="center">
					...
					<tr>
						<td colspan="2" align="center"><div><b>글내용</b></div></td>
						<td colspan="2"><div><b style="color: #0869CA;">답변 비교</b></div></td>
					</tr>
					<tr  height="230">
						<td colspan="2" align="left">${fn:replace(board.content, newLineChar,"<br>")}
						<td colspan="2" align="left" 
						style="border:6px; border-style:inset; border-color:silver; border-collapse: collapse; background-color:#f5fffa;"><!-- 베이지색 : #fff5ee -->
						<div id="replyup"></div></td>
					...
					<tr>
						...
						|<button class="btn btn-primary rounded-pill m-2" onclick="location.href='boardDelete.user?boardname=${board.boardname}&idx=${idx}&cp=${cpage}&ps=${pagesize}&writerid=${board.userid}'">삭제</button>
                     ...
                     <table width="90%"  >
                           ...
                           <tr align="left" >
                              <td width="80%" seq2="${reply.replynum}">[${reply.userid}] : ${reply.content} <br>
                                 <small><small>작성일:${reply.replydate}</small></small>
                                 <c:forEach var="replyAnswerList" items="${replyAnswerList}">
                                    <c:if test="${reply.replynum eq replyAnswerList.parentreply}">
                                        <div class="nav-item dropdown">
                                           <button class="btn btn-primary rounded-pill m-2 nav-link dropdown-toggle" data-bs-toggle="dropdown" seq="${reply.replynum}" idx="${idx}" id="reply_comment">답글</button>
                                        </div>
                                    </c:if>
                                 </c:forEach> 
                              </td>
                              <c:if test="${sessionScope.userid eq reply.userid}">
                                 <td>
                                    ...
                                    <button class="btn btn-primary rounded-pill m-2" seq="${reply.replynum}" idx="${idx}" id="reply_answer">답변댓글</button>
									<button class="btn btn-primary rounded-pill m-2"  seq="${reply.content}" idx="${idx}" id="replycompare">비교하기</button>
                                 </td>
                              </c:if>
                           </tr>
                     </table>
