...
public class CommonDao {
	...
	public List<Board> hotBoardList() {
		...
		try {
			...
			sql1 = "select boardname, title, idx,userid, lovenum from "
					+ "(select boardname, title,idx,userid,lovenum from board order by lovenum desc) where rownum <= 3";
			pstmt1 = conn.prepareStatement(sql1);
	...
	public List<Board> noticeBoardList() {
		...
		try {
			conn = ds.getConnection();
			sql1 = "select boardname, title, idx,userid, lovenum from "
					+ "(select boardname, title,idx,userid,lovenum from board order by lovenum desc) where rownum <= 3 and boardname='공지사항'";
			pstmt1 = conn.prepareStatement(sql1);
	...
	public List<Board> getBoardSearch(int cpage, int pagesize, String searchWord) {
		List<Board> boardsList = new ArrayList<Board>(); // return 값을 boardsList
		sql4 = new StringBuffer();
		...
		try {
			...
			System.out.println("getBoardSearch sql4: " + sql4);
			...
			rs1 = pstmt1.executeQuery(); // SELECT 할때 쓰는 함수
			System.out.println("rs1: " + rs1.toString());

			while (rs1.next()) {
				System.out.println("while문 들어왔다");

				Board board = new Board(); // 배열에 하나씩 들어갈 아이
				board.setIdx(rs1.getInt("idx"));
				board.setUserid(rs1.getString("userid")); // writerid
				board.setBoardname(rs1.getString("boardname"));
				board.setTitle(rs1.getString("title"));
				/*
