				...
				<table class="table" width="80%" cellspacing="0" align="center">
					...
					<c:forEach var="category" items="${categoryList}">
						<tr onmouseover="this.style.backgroundColor='skyblue'"
							...
							<td align="center" colspan="1"><a
								href="boardList.user?boardname=${category.boardname}">${category.boardname}</a>
							</td>
							<td colspan="1"><c:forEach var="count" items="${count}">
									<c:if test="${count.key eq category.boardname}">
							${count.value}
							</c:if>
								</c:forEach></td>


							<td colspan="3"><button type="button" class="btn btn-primary rounded-pill m-2" 	onclick="location.href='AdminBoardDelete.admin?boardname=${category.boardname}'">삭제</button></td>
						</tr>
					...
					<tr>
						...
						<td colspan="3"><button type="button"
								...
								onclick="location.href='AdminAllBoardDelete.admin';">삭제</button></td>
					</tr>
<td colspan="3"><button type="button" class="btn btn-primary rounded-pill m-2" 	onclick="location.href='AdminBoardReturn.admin'">복구하기</button></td>
				</table>
			</div>
		</div>
		<!-- Footer Start -->
