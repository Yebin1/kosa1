...
<body>
	...
	<c:forEach var="replyAnswerList" items="${replyAnswerList}">
		<c:set var="test"
			value="${replyAnswerList.enddate - replyAnswerList.startdate }" />
	</c:forEach>
	...
	<div id="pagecontainer">
		<div class="bg-light rounded h-100 p-4">
			<h4 class="mb-4">
				<b><span style="color: #2C91F7;">${boardname}</span>게시판</b>
			</h4>
			<br>
			<table class="table">
				<tr>
					<td colspan="5">
						<form name="boardheader" action='HeaderSearchService.user'>
							...
							</select> <input type="hidden" name="boardname" value="${boardname}">
						</form>
					<td colspan="6">
						<form name="boardList">
							<select class="form-select form-select-sm mb-3"
								aria-label=".form-select-sm example" name="ps"
								onchange="submit()" style="width: 100px;">
				...
				<c:forEach var="board" items="${boardList}">
					<tr onmouseover="this.style.backgroundColor='skyblue'"
						...
						<c:forEach var="recruitboard" items="${recruitboardList}">
							<c:if test="${recruitboard.idx eq board.idx}">
								<td><small>${recruitboard.startdate}~<br>${recruitboard.enddate}</small></td>
							</c:if>
						</c:forEach>
						<td>${board.lovenum}</td>
						<td>${board.readnum}</td>
				...
				<tr>
					<td colspan="3" align="center">
						<!--이전 링크 --> <c:if test="${cpage > 1}">
							<a
								href="boardList.user?boardname=${boardname}&cp=${cpage-1}&ps=${pagesize}&boardname=${boardname}&boardheader=${boardheader}&totalboardcount=${totalboardcount}">이전</a>
							...
							<c:choose>
								...
								<c:otherwise>
									<a
										href="boardList.user?boardname=${boardname}&cp=${i}&ps=${pagesize}&boardname=${boardname}&boardheader=${boardheader}&totalboardcount=${totalboardcount}">[${i}]</a>
								</c:otherwise>
							</c:choose>
						</c:forEach> <!--다음 링크 --> <c:if test="${cpage < pagecount}">
							<a
								href="boardList.user?boardname=${boardname}&cp=${cpage+1}&ps=${pagesize}&boardname=${boardname}&boardheader=${boardheader}&totalboardcount=${totalboardcount}">다음</a>
						</c:if>
					</td>
	...
	<div>
		...
		<button type="button" class="btn btn-primary rounded-pill m-2"
			onclick="location.href='boardWrite.user?boardname=${boardname}';">글쓰기</button>
	</div>
