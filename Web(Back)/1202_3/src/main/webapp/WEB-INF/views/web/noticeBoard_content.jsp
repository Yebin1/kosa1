...
<script src="https://kit.fontawesome.com/21b939066c.js"
	crossorigin="anonymous"></script>
...
<body>
	<c:choose>
		...
		<c:when
			test="${sessionScope.userid ne null and sessionScope.userid ne admin}">
			<!-- 일반사용자 -->
			<!-- 사이드바 들어갈 자리 -->
				...
				<div style="padding-top: 25px; text-align: center; width: 89%;">
					...
					<table class="table" width="95%" align="center">
						...
						<tr>
							...
							<td width="30%" id="bookmarking"><jsp:include
									page="/WEB-INF/views/web/bookMark.jsp"></jsp:include></td>
						...
						<tr>
							...
							<td><c:choose>
									...
									<button class="btn btn-primary rounded-pill m-2" type="button"
											href="#" id="fileDetail">상세보기</button>
									</c:when>
									<c:otherwise>
									${filename}<button class="btn btn-primary rounded-pill m-2"
											type="button" href="#" id="fileDetail">상세보기</button>
									</c:otherwise>
								</c:choose>
								<div id="fileList"></div> <!-- style="display: none; --></td>
							<td width="20%" align="center"><b>말머리</b></td>
							</td>
							<td>
								<div class="col-sm-12 col-md-6 col-xl-4"
									style="margin-left: 70px;">
									<div style="color: red;">[${board.boardheader}]</div>
								</div>
							</td>
						</tr>
						<%-- <tr>
						...
						<tr height="230">
   						...
   						<tr>
                                <c:choose>
                              <c:when test="${sessionScope.userid eq 'admin' && board.boardstatus eq 0}"> 
                              <td colspan="4" align="center">
                              <button class="btn btn-primary rounded-pill m-2"
                              onclick="location.href='AdminBoardReturn.admin'">돌아가기</button></td>
                              </c:when>
								...
								<td align="left"><input type="hidden" id="reply_userid"
									...
									<div class="form-floating">
										<textarea class="form-control"
											placeholder="Leave a comment here" id="reply_content"
											style="height: 150px;"></textarea>
										<label for="floatingTextarea">Comments</label>
									</div> <br>
									<button class="btn btn-primary rounded-pill m-2" type="button"
										id="reply_check">등록</button></td>
							</tr>
					...
					<c:if test="${not empty replyList}">
						<c:forEach var="reply" items="${replyList}">
							<c:if test="${reply.parentreply eq 0}">
								<div id="clickDiv" align="center"
									style="width: 90%; border-radius: 50px; background-color: #E3F1FF; margin-bottom: 20px;">
									<table width="90%">
										<tr>
											<th colspan="2">[${reply.userid}]&ensp; <small><small>작성일:
														${reply.replydate}</small></small></th>
										</tr>
										<tr align="left">
											<td width="80%" seq2="${reply.replynum}">${reply.content}
												<br> <c:forEach var="replyAnswerList"
													items="${replyAnswerList}">
													<c:if
														test="${reply.replynum eq replyAnswerList.parentreply}">
														<div class="nav-item dropdown">
															<button
																class="btn btn-primary rounded-pill m-2 nav-link dropdown-toggle"
																data-bs-toggle="dropdown" seq="${reply.replynum}"
																idx="${idx}" id="reply_comment"
																style="background-color: transparent; color: #17A4FF;">
																<small>답글</small>
															</button>
														</div>
													</c:if>
												...
												<c:when test="${sessionScope.userid eq reply.userid}">
													<td align="right">
														...
														<button type="button" class="btn btn-outline-primary"
															seq="${reply.replynum}" idx="${idx}" id="reply_modify">
															<small>수정</small>
														</button>
														<button type="button" class="btn btn-outline-primary"
															seq="${reply.replynum}" idx="${idx}" id="reply_del">
															<small>삭제</small>
														</button>
														<button type="button" class="btn btn-outline-primary"
															seq="${reply.replynum}" idx="${idx}" id="reply_answer">
															<small>답글달기</small>
														</button>
														<button type="button" class="btn btn-outline-primary"
															seq="${reply.content}" idx="${idx}" id="replycompare">
															<small>비교하기</small>
														</button>
