...
<body>
	...
	<div id="pagecontainer">
		<div class="bg-light rounded h-100 p-4">
			<h4 class="mb-4">
				<b>내가 쓴<span style="color: #2C91F7;"> 댓글</span></b>
			</h4>
			<br>
			<table class="table" width="80%" cellspacing="0" align="center">
				...
				<c:forEach var="reply" items="${reply}" varStatus="status">
					<tr onmouseover="this.style.backgroundColor='skyblue'"
						onmouseout="this.style.backgroundColor='white'"></tr>
					<td align="center">${board[status.index].boardname}</td>
					<td align="center">${board[status.index].boardheader}</td>
					<td align="left"><a
						href="boardContent.user?boardname=${board[status.index].boardname}&idx=${board[status.index].idx}&cp=${cpage}&ps=${pagesize}">
							<c:choose>
								<c:when
									test="${board[status.index].title != null && fn:length(board[status.index].title) > 20}">
										${fn:substring(board[status.index].title,0,20)}...
									</c:when>
								<c:otherwise>
										${board[status.index].title}
									</c:otherwise>
							</c:choose>
					</a></td>
					...
					<td align="center">${reply.replydate}</td>
	...
	<div>
		<a class="btn btn-primary rounded-pill m-2" href="myboardList.user">
			내 글 보기</a>
	</div>
	<!-- Footer Start -->
