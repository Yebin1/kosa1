...
public class MemberDao {
	...
	// 노현중 하는중 nhj
	// 글쓰기(프로젝트 모집 추가)
	public int projectWrite(Recruitboard recruitboard, Board board) {
		...
		try {
			...
			sql1 = "insert all "
					+ "into board(idx,userid,boardname,title,content,writedate,readnum,lovenum,boardstatus,boardheader) "
					+ "values(board_idx.nextval,?,?,?,?,sysdate,0,0,1,?) "
					+ "into recruitboard(idx,startdate,enddate,latitudemap,logitudemap,apply,placename) "
					+ "values(board_idx.currval,?,?,?,?,?,?) " + "select * from dual";

			pstmt1 = conn.prepareStatement(sql1);
			pstmt1.setString(1, board.getUserid());
			pstmt1.setString(2, board.getBoardname());
			pstmt1.setString(3, board.getTitle());
			pstmt1.setString(4, board.getContent());
			pstmt1.setString(5, board.getBoardheader());
			pstmt1.setDate(6, (Date) recruitboard.getStartdate());
			pstmt1.setDate(7, (Date) recruitboard.getEnddate());
			pstmt1.setDouble(8, recruitboard.getLatitudemap());
			pstmt1.setDouble(9, recruitboard.getLogitudemap());
			pstmt1.setString(10, recruitboard.getApply());
			pstmt1.setString(11, recruitboard.getPlacename());
	...
	public int projectWrite(Recruitboard recruitboard, Board board, Files file) {
		try {
			conn = ds.getConnection();
			conn.setAutoCommit(false);
			...
			sql1 = "insert all "
					...
					+ "values(board_idx.nextval,?,?,?,?,sysdate,0,0,1,?) "
					+ "into recruitboard(idx,startdate,enddate,latitudemap,logitudemap,apply,placename) "
					+ "values(board_idx.currval,?,?,?,?,?,?) " + "into files(filenum,idx,filename,filesize) "
					+ "values(files_idx.nextval,board_idx.currval,?,?) " + "select * from dual";
			pstmt1 = conn.prepareStatement(sql1);
			pstmt1.setString(1, board.getUserid());
			pstmt1.setString(2, board.getBoardname());
			pstmt1.setString(3, board.getTitle());
			...
			pstmt1.setString(10, recruitboard.getApply());
			pstmt1.setString(11, recruitboard.getPlacename());
			pstmt1.setString(12, file.getFilename());
			pstmt1.setInt(13, file.getFilesize());
	...
	public List<Bookmarks> BookmarksList(int cpage, int pagesize, String userid) {
		sql1 = "SELECT *  FROM " + "(SELECT ROWNUM RNUM, bb.*  FROM "
				+ "(SELECT bookmarks.userid, bookmarks.idx, bookmarks.writerid, bookmarks.title, bookmarks.writedate, bookmarks.boardname, board.boardstatus FROM bookmarks, board "
				+ "where bookmarks.idx=board.idx and boardstatus=1 and bookmarks.userid=? " + "ORDER BY idx DESC) bb) "
				+ "WHERE RNUM <=?  AND RNUM >=?";
	...
	public List<Board> myList(int cpage, int pagesize, String userid) {
		try {
			conn = ds.getConnection();
			sql1 = "SELECT *  FROM (SELECT ROWNUM RNUM, board.*  FROM "
					+ "(SELECT * FROM board where userid=? and boardstatus=1 ORDER BY idx DESC) board) "
					+ "WHERE RNUM <=?  AND RNUM >=?";
			pstmt1 = conn.prepareStatement(sql1);
	...
	// recruitboard 출력 List

	public List<Recruitboard> recruitboardList(int idx) {
		List<Recruitboard> recruitboardList = new ArrayList<>();
		try {

			conn = ds.getConnection();
			sql1 = "select idx,startdate,enddate,latitudemap,logitudemap,apply,placename from recruitboard where idx = ?";
			pstmt1 = conn.prepareStatement(sql1);
			pstmt1.setInt(1, idx);
			rs1 = pstmt1.executeQuery();
			while (rs1.next()) {
				Recruitboard recruitboard = new Recruitboard();
				recruitboard.setStartdate(rs1.getDate("startdate"));
				recruitboard.setEnddate(rs1.getDate("enddate"));
				recruitboard.setLatitudemap(rs1.getDouble("latitudemap"));
				recruitboard.setLogitudemap(rs1.getDouble("logitudemap"));
				recruitboard.setApply(rs1.getString("apply"));
				recruitboard.setPlacename(rs1.getString("placename"));
				recruitboardList.add(recruitboard);
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public List<Recruitboard> recruitboardList() {
		List<Recruitboard> recruitboardList = new ArrayList<>();
		try {

			conn = ds.getConnection();
			sql1 = "select idx,startdate,enddate,latitudemap,logitudemap,apply from recruitboard";
			pstmt1 = conn.prepareStatement(sql1);
			rs1 = pstmt1.executeQuery();
			while (rs1.next()) {
				Recruitboard recruitboard = new Recruitboard();
				recruitboard.setIdx(rs1.getInt("idx"));
				recruitboard.setStartdate(rs1.getDate("startdate"));
				recruitboard.setEnddate(rs1.getDate("enddate"));
				recruitboard.setLatitudemap(rs1.getDouble("latitudemap"));
				recruitboard.setLogitudemap(rs1.getDouble("logitudemap"));
	...
	// 마이페이지 댓글 리스트 (reply)
	public List<Reply> myReplyList(int cpage, int pagesize, String userid) {
		try {
			conn = ds.getConnection();

			sql1 = "select * from (select rownum rnum, rb.* from "
					+ "(select reply.replydate, reply.content, reply.replynum, board.boardname, board.title, board.boardheader from reply, board where reply.idx=board.idx "
			...
			while (rs1.next()) {
				...
				Reply reply = new Reply();
				reply.setReplynum(rs1.getInt("replynum"));
				reply.setContent(rs1.getString("content"));
				reply.setReplydate(rs1.getDate("replydate"));
				replyList.add(reply);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	// 마이페이지 댓글 리스트 (board)
	public List<Board> myReplyListBoard(int cpage, int pagesize, String userid) {
		try {
			conn = ds.getConnection();

			sql1 = "select * from (select rownum rnum, rb.* from "
					+ "(select reply.replydate, reply.content, reply.replynum, board.idx, board.boardname, board.title, board.boardheader from reply, board where reply.idx=board.idx "
					+ "and reply.replystatus=1 and reply.userid=? order by reply.replynum desc) rb)"
					+ "	where rnum <= ?  and rnum >=?";
	...
	public List<Board> headerSearch(int cpage, int pagesize, String boardheader) {
		...
		try {
			...
			sql1 = "select * from (select rownum rnum, b.* from "
					...
					+ "where boardheader = ? and boardstatus =1 order by idx desc) b) "
					+ "where rnum >=? and rnum <=?";
	...
	// AD 파일 업로드 하기
	public int ADUploadOk(AdFiles adfile) {
		try {
			conn = ds.getConnection();

			if (rs1.next()) {
				sql1 = "insert into adfiles(adfilenum, adfilename, adfilesize) " + "values(adfiles_idx.nextval,?,?)";
				pstmt1 = conn.prepareStatement(sql1);
				pstmt1.setString(1, adfile.getAdfilename());
				pstmt1.setInt(2, adfile.getAdfilesize());
				resultRow = pstmt1.executeUpdate();
			}
