...
import kr.or.kosa.service.user.MapApiViewService;
...
public class UserController extends HttpServlet {
	...
	private void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		if (urlcommand.equals("/index.user")) { // 메인페이지(홈버튼)
			...
			forward = action.execute(request, response);
		}else if (urlcommand.equals("/boardMap.user")) {   //지도 api
			action = new BoardMapAPI();
			forward = action.execute(request, response);
		}else if (urlcommand.equals("/boardProjectWrite.user")) {// 프로젝트 게시글 작성
			action = new BoardProjectWriteService();
			forward = action.execute(request, response);
		}else if (urlcommand.equals("/MapApiView.user")) {// 프로젝트 게시글 작성
			action = new MapApiViewService();
			forward = action.execute(request, response);
		}
