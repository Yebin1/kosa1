...
import kr.or.kosa.service.admin.AdminBoardReturnListService;
import kr.or.kosa.service.admin.AdminBoardReturnOkService;
...
import kr.or.kosa.service.admin.SearchUseridService;
...
public class AdminController extends HttpServlet {
	...
	private void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		if (urlcommand.equals("/adminMain.admin")) { // 어드민 메인
		...
		} else if (urlcommand.equals("/stop.admin")) { // 계정 정지
			action = new MemberStopService();
			forward = action.execute(request, response);
		} else if (urlcommand.equals("/activate.admin")) { // 계정 정지
			...
			forward = action.execute(request, response);
		} else if (urlcommand.equals("/AdminBoardReturn.admin")) { // 관리자 삭제대기 게시글 리스트로 이동
			action = new AdminBoardReturnListService();
			forward = action.execute(request, response);
		} else if (urlcommand.equals("/AdminBoardReturnOk.admin")) { // 게시글 복구
			action = new AdminBoardReturnOkService();
			forward = action.execute(request, response);
		}else if (urlcommand.equals("/searchUserid.admin")) {// 사용자 아이디로 조회
			action = new SearchUseridService();
			forward = action.execute(request, response);
		}  else if (urlcommand.equals("/activation.admin")) {// 활성화 or 비활성화(toggle)
			action = new MemberActivateService();
	  		forward = action.execute(request, response);
		}else if (urlcommand.equals("/activation_delete.admin")) {// 활성화 or 비활성화(toggle)
			action = new SearchUseridService();
	  		forward = action.execute(request, response);
		}
